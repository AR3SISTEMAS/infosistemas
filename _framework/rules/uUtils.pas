...
uses System.SysUtils, System.Classes, Forms, Vcl.StdCtrls, Dialogs, Windows, System.RegularExpressions, Winapi.WinInet;
...
const ANone : string = 'NONE';
var LState : Dword;
    LConn  : string;
begin
  Result := False;
  LConn  := ANone;
  InternetGetConnectedState(@LState, 0);
  Sleep(500);
  if LState and INTERNET_CONNECTION_LAN   <> 0 then
    LConn := 'LAN';
  if LState and INTERNET_CONNECTION_MODEM <> 0 then
    LConn := 'MODEM';
  if LState and INTERNET_CONNECTION_PROXY <> 0 then
    LConn := 'PROXY';
  Result := LConn <> ANone;
end;
